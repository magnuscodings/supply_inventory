<?php 
include("include/authentication/user-status.php");
...
<?php include("include/navbar-user.php")?>
      <section>
<?php include("include/user/user-add-request.php")?>
<!-- Search bar -->
...
<table border="1px" class="table-responsive table-bordered table-sm mt-3 text-center">
  ...
  <?php
      if(isset($_POST['Search-Item'])){
          ...
          $search="SELECT a.*,b.*
          FROM custodian as a LEFT JOIN items as b ON a.itemID = b.id where a.userID ='$id' AND (
            b.description='$search_item' or a.userID like '$search_item' or b.description like '$search_item'  or a.location like '$search_item' or a.remarks like '$search_item'
          )";
      ...
      <tr>
        ...
        <th><?php echo "P".$row['value']*$row['quantity']?></th>
 ...
 <?php
        ...
        $select="SELECT a.*,b.*
         FROM custodian as a LEFT JOIN items as b ON a.itemID = b.id where a.userID ='$id'";
 ...
 <?php
        ...
        echo "<tr><td colspan='9' class='text-danger'>0 results</td></tr>";
      }
    }
  ?>
